...
function useSetupAnimatedValues() {
  ...
  const touchPositionDiffConstrained = useDerivedValue(() => {
    var _props$headerOffset, _props$headerOffset2;
    ...
    const constrained = Math.max((_props$headerOffset = props.headerOffset) !== null && _props$headerOffset !== void 0 ? _props$headerOffset : 0, Math.min(containerMinusActiveCell, Math.max(scrollOffset.value, offsetRelativeToScrollTop)));
    const maxTranslateNegative = -activeCellOffset.value + ((_props$headerOffset2 = props.headerOffset) !== null && _props$headerOffset2 !== void 0 ? _props$headerOffset2 : 0);
